...
// Effective overlay row of a category's sub-items (advancement criteria and multi-stat sub-stats):
// Row 1, or 0 (no row) when they never reach it. A simple stat's auto-generated criterion is never
// on Row 1, and a multi-stat can opt its sub-stats out via hide_substats_in_row1, so "r1"/"row1"
// must not turn either of them up.
static int subitem_effective_row(const TrackableCategory *cat) {
    if (cat->is_single_stat_category || cat->hide_substats_in_row1) return 0;
    return 1;
}

// Text-only category match (no indicator keywords). Feeds category_reveals_children() below.
...
// True when the search is one of the multi-stat keywords ("multi", "nor1", ...) and this category
// answers to it. Every other flag is passed as unset so only those two can match here, which keeps
// the keyword aliases defined in exactly one place.
static bool category_substat_keyword_matches(const TrackableCategory *cat, const char *search,
                                             bool is_stat_section) {
    if (!is_stat_section || cat->is_single_stat_category) return false;
...
/**
 * @brief Pulls a boolean flag out of the head of a SpeedrunIGT record.json.
 *
 * Same idea as speedrunigt_parse_record_head(): the flags sit near the start of the record, so the
 * whole file never has to be parsed. Returns false when the key is missing from the chunk or when
 * its value is cut off by the end of it, in which case the caller falls back to a full JSON parse.
 *
 * @param head A NUL-terminated chunk from the start of the file.
 * @param key The quoted key to look for, for example "\"is_completed\"".
 * @param out_value Receives the flag.
 * @return True when a usable value was extracted.
 */
static bool speedrunigt_parse_record_head_bool(const char *head, const char *key, bool *out_value) {
    const char *found = strstr(head, key);
 ...
 *
 * The record's "is_completed" flag comes along with it. It tells the two cases apart that the time
 * alone cannot: a record that simply hasn't been rewritten yet, and one the mod considers final
 * because its timer stopped.
 *
 * @param t The tracker, used for the saves path and current world name.
 * @param out_ms Receives the final IGT in milliseconds.
 * @param out_completed Receives the record's "is_completed" flag, false when the record has none.
 * @return True when a record with a usable final IGT was found.
 */
static bool tracker_read_speedrunigt_final_ms(const Tracker *t, long long *out_ms, bool *out_completed) {
    if (!t || !out_ms || !out_completed) return false;
    *out_completed = false;
    ...
    // The mod writes a fixed header before any of the big arrays: "is_completed" on line 6 and
    // "final_igt" on line 13, with "world_name" as the only field that can grow. A kilobyte covers
    // that header several times over, so the rest of the record is never even read.
    char head[1024];
    size_t read_len = fread(head, 1, sizeof(head) - 1, file);
    fclose(file);
    head[read_len] = '\0';

    bool head_has_igt = speedrunigt_parse_record_head(head, out_ms);
    bool head_has_completed = speedrunigt_parse_record_head_bool(head, "\"is_completed\"", out_completed);
    if (head_has_igt && head_has_completed) return true;

    // A record that doesn't look like the mod's usual header (different key order, or a world name
    // long enough to push the keys out of the chunk): fall back to parsing the whole file.
    cJSON *record = cJSON_from_file(record_path);
    if (!record) return head_has_igt;

    bool success = head_has_igt;
    ...
    const cJSON *is_completed = cJSON_GetObjectItem(record, "is_completed");
    if (cJSON_IsBool(is_completed)) {
        *out_completed = cJSON_IsTrue(is_completed);
    }
 ...
 *
 * When the mod's timer stops on a run the tracked template counts as complete too, the record says
 * so with "is_completed": true and its final_igt is the run's real final time. That time is then
 * held from the moment the record reports it, instead of waiting for the next game save to freeze
 * on a play time that has already moved past the end of the run. Until the record says it is
 * completed, the display keeps following the mod as usual.
 */
static void tracker_refresh_igt(Tracker *t, const AppSettings *settings) {
    if (!t || !t->template_data) return;
    TemplateData *td = t->template_data;

    long long igt_ms = 0;
    bool record_completed = false;
    if (!tracker_read_speedrunigt_final_ms(t, &igt_ms, &record_completed)) {
        ...
        if (igt_ms != t->speedrunigt_last_ms) {
            ...
            t->speedrunigt_stalled = false;
            t->speedrunigt_final = false; // A record that moves again is not a final time
            t->speedrunigt_last_ms = igt_ms;
            t->speedrunigt_last_save_ticks = td->play_time_ticks;
        } else if (!t->speedrunigt_final && td->play_time_ticks > t->speedrunigt_last_save_ticks) {
        ...
        // The mod finished ITS run and the tracked template counts as complete too, so this record
        // carries the run's final time and it is held from here on, instead of waiting for the next
        // game save. Only latch it while the timer is supposed to freeze at all, and never once the
        // mod's timer was found stopped on a run that kept going: that case belongs to the stats
        // file, which freezes on the play time of the next save as usual.
        if (record_completed && td->run_completed && settings && settings->igt_freeze_on_completion &&
            !t->speedrunigt_stalled) {
            if (!t->speedrunigt_final) {
                log_message(LOG_INFO,
                            "[TRACKER] SpeedrunIGT reported a completed run, freezing the IGT at %lld ms.\n",
                            igt_ms);
            }
            t->speedrunigt_final = true;
            td->frozen_ticks_pending = false; // The final time is known, no game save has to confirm it
        } else if (t->speedrunigt_final && !(settings && settings->igt_freeze_on_completion)) {
            // Freezing was turned off again, so let the timers carry on from the stats file.
            t->speedrunigt_final = false;
        }
...
// True if the screen-space rect [mn, mx] overlaps the display at all. Used for per-element
// culling: a draw is skipped only when its exact rect lies fully outside the viewport.
static bool rect_on_screen(const ImVec2 &mn, const ImVec2 &mx, const ImVec2 &display_size) {
...
static float tracker_cached_name_width(const char *name, float &cache_w, float &cache_font,
                                       float font_size, float base_size) {
    if (cache_font != font_size) {
        ImGui::SetWindowFontScale(base_size > 0.0f ? (font_size / base_size) : 1.0f);
        cache_w = ImGui::CalcTextSize(name).x;
        ImGui::SetWindowFontScale(1.0f);
        cache_font = font_size;
    }
    return cache_w;
}

// Like tracker_cached_name_width but for text that changes at runtime (progress/stage strings).
// Re-measured only when the text content or the font size changes.
static float tracker_cached_text_width(const char *text, char *cache_text, int cache_text_size,
                                       float &cache_w, float &cache_font, float font_size, float base_size) {
    if (cache_font != font_size || strcmp(text, cache_text) != 0) {
        ImGui::SetWindowFontScale(base_size > 0.0f ? (font_size / base_size) : 1.0f);
        cache_w = ImGui::CalcTextSize(text).x;
        ImGui::SetWindowFontScale(1.0f);
        cache_font = font_size;
        strncpy(cache_text, text, (size_t) cache_text_size - 1);
        cache_text[cache_text_size - 1] = '\0';
    }
    return cache_w;
}
...
bool tracker_get_current_element_pos(Tracker *t, const char *section, const char *root_name,
    ...
    if (!t || !t->template_data || !section || !root_name || !element || !out_x || !out_y) return false;
    TemplateData *td = t->template_data;
    bool has_parent = parent_root_name && parent_root_name[0] != '\0';

    auto pick = [&](ManualPos *icon, ManualPos *text, ManualPos *prog) -> ManualPos *{
        if (strcmp(element, "icon") == 0) return icon;
        if (strcmp(element, "text") == 0) return text;
        if (strcmp(element, "progress") == 0) return prog;
        return nullptr;
    };

    auto find_in_categories = [&](TrackableCategory **arr, int count) -> ManualPos *{
        const char *wanted = has_parent ? parent_root_name : root_name;
        for (int i = 0; i < count; i++) {
            TrackableCategory *cat = arr[i];
            if (!cat || strcmp(cat->root_name, wanted) != 0) continue;
            if (!has_parent) return pick(&cat->icon_pos, &cat->text_pos, &cat->progress_pos);
            for (int j = 0; j < cat->criteria_count; j++) {
                TrackableItem *crit = cat->criteria[j];
                if (crit && strcmp(crit->root_name, root_name) == 0)
                    return pick(&crit->icon_pos, &crit->text_pos, &crit->progress_pos);
            }
            return nullptr;
        }
        return nullptr;
    ...
    auto find_in_items = [&](TrackableItem **arr, int count) -> ManualPos *{
        for (int i = 0; i < count; i++) {
            if (arr[i] && strcmp(arr[i]->root_name, root_name) == 0)
                return pick(&arr[i]->icon_pos, &arr[i]->text_pos, &arr[i]->progress_pos);
        }
        return nullptr;
    };

    ManualPos *pos = nullptr;
    if (strcmp(section, "advancement") == 0) {
        pos = find_in_categories(td->advancements, td->advancement_count);
    } else if (strcmp(section, "stat") == 0) {
        pos = find_in_categories(td->stats, td->stat_count);
    } else if (strcmp(section, "unlock") == 0) {
        pos = find_in_items(td->unlocks, td->unlock_count);
    } else if (strcmp(section, "custom") == 0) {
        pos = find_in_items(td->custom_goals, td->custom_goal_count);
    } else if (strcmp(section, "multi_stage") == 0) {
        for (int i = 0; i < td->multi_stage_goal_count && !pos; i++) {
            MultiStageGoal *goal = td->multi_stage_goals[i];
            if (goal && strcmp(goal->root_name, root_name) == 0)
                pos = pick(&goal->icon_pos, &goal->text_pos, &goal->progress_pos);
        }
    } else if (strcmp(section, "counter") == 0) {
        for (int i = 0; i < td->counter_goal_count && !pos; i++) {
            CounterGoal *goal = td->counter_goals[i];
            if (goal && strcmp(goal->root_name, root_name) == 0)
                pos = pick(&goal->icon_pos, &goal->text_pos, &goal->progress_pos);
        }
    }

    if (!pos || !pos->auto_valid) return false;

    ImVec2 anchor_off = get_anchor_offset(anchor, pos->auto_w, pos->auto_h);
    *out_x = fminf(fmaxf(roundf(pos->auto_x - anchor_off.x), -MANUAL_POS_MAX), MANUAL_POS_MAX);
    *out_y = fminf(fmaxf(roundf(pos->auto_y - anchor_off.y), -MANUAL_POS_MAX), MANUAL_POS_MAX);
    return true;
}
...
// Records the world-space rect an element occupies, so the template editor can seed manual
// coordinates with the spot the element currently has. Elements that are drawn get this from their
// drag handle; hidden ones have to record it themselves, which is only worth doing while the
// template editor is open (tracker_layout_capture_active()).
static void record_auto_layout_rect(const Tracker *t, ManualPos &pos, ImVec2 screen_pos, ImVec2 size) {
    pos.auto_x = (screen_pos.x - t->camera_offset.x) / t->zoom_level;
    pos.auto_y = (screen_pos.y - t->camera_offset.y) / t->zoom_level;
    pos.auto_w = size.x / t->zoom_level;
    pos.auto_h = size.y / t->zoom_level;
    pos.auto_valid = true;
}

// True while the template editor is open, where positions of hidden elements are worth measuring.
// Everywhere else, hidden elements stay as cheap as they have always been.
static inline bool tracker_layout_capture_active(const Tracker *t) {
...
static void handle_visual_layout_dragging(Tracker *t, const char *id, ImVec2 item_screen_pos, ImVec2 hit_box_size,
    ...
    // Remember where this element actually rendered, so the template editor can seed manual
    // coordinates with the position the element currently has instead of a fixed default.
    record_auto_layout_rect(t, target_pos, item_screen_pos, hit_box_size);
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (cat->criteria_count > 0 && !cat->is_single_stat_category) {
            ...
            for (int j = 0; j < cat->criteria_count; ++j) {
                ...
                if (item_matches_search(crit, t->search_buffer, subitem_effective_row(cat))) {
                ...
                } else if (parent_reveals_children) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!is_simple_stat_category && cat->criteria_count > 0) {
                for (int j = 0; j < cat->criteria_count; j++) {
                    ...
                    bool crit_matches_search = parent_reveals_children
                                               || (child_matches_render && item_matches_search(
                                                       crit, t->search_buffer, subitem_effective_row(cat)))
                    ...
                    if (crit && !crit_should_hide_width && crit_matches_search) {
                        ...
                        float crit_text_width = tracker_cached_name_width(crit->display_name, crit->cached_name_w,
                                                                          crit->cached_name_w_font,
                                                                          sub_font_size, t->tracker_font->LegacySize);
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!parent_reveals_children) {
                for (int j = 0; j < cat->criteria_count; j++) {
                    ...
                    bool crit_search =
                            item_matches_search(crit, t->search_buffer, subitem_effective_row(cat));
                    std::string crit_link_key = std::string(cat->root_name) + "\t" + crit->root_name;
            ...
            // A fully hidden parent takes its sub-items with it, but only the ones that ride along with
            // it. Sub-item elements with their own position are independent and stay visible.
            bool parent_fully_hidden_in_layout = hide_icon_in_layout && hide_text_in_layout &&
                                                 (!has_progress_text || hide_progress_in_layout);
            bool has_independent_child_element = false;
            if (parent_fully_hidden_in_layout && is_complex) {
                for (TrackableItem *crit: children_to_render) {
                    if (!crit) continue;
                    bool crit_hiding_active = tracker_layout_hiding_active(settings, crit->done);
                    if ((crit->icon_pos.is_set && !(crit->icon_pos.is_hidden_in_layout && crit_hiding_active)) ||
                        (crit->text_pos.is_set && !(crit->text_pos.is_hidden_in_layout && crit_hiding_active)) ||
                        (crit->progress_pos.is_set && !(crit->progress_pos.is_hidden_in_layout && crit_hiding_active))) {
                        has_independent_child_element = true;
                        break;
                    }
                }
            }
            ...
            if (parent_fully_hidden_in_layout && !cat->icon_pos.is_set && !has_independent_child_element) {
                continue;
            }
            ...
            } else if (parent_fully_hidden_in_layout) {
                // Only alive for its independently placed sub-items, so it must not leave a hole in the grid
            } else {
            ...
            if (is_visible_on_screen) {
                ...
                // Get Text Sizes for Centering (only measure if the text will be drawn; is_complex needs it for criteria layout)
                bool show_main_text = (t->zoom_level > LOD_TEXT_MAIN_THRESHOLD) && !hide_text_in_layout;
                bool show_snapshot_text = has_snapshot_text && !hide_text_in_layout &&
                                          (t->zoom_level > LOD_TEXT_MAIN_THRESHOLD);
                bool show_progress_text = has_progress_text && !hide_progress_in_layout &&
                                          (t->zoom_level > LOD_TEXT_SUB_THRESHOLD);
                bool need_text_sizes = show_main_text || show_snapshot_text || show_progress_text || is_complex;

                ImVec2 text_size = ImVec2(0.0f, 0.0f);
                ImVec2 progress_text_size = ImVec2(0.0f, 0.0f);
                ImVec2 snapshot_text_size = ImVec2(0.0f, 0.0f);
                if (need_text_sizes) {
                    SET_FONT_SCALE(settings->tracker_font_size, t->tracker_font->LegacySize);
                    text_size = ImGui::CalcTextSize(cat->display_name);
                    RESET_FONT_SCALE();
                    ...
                    snapshot_text_size = ImGui::CalcTextSize(snapshot_text);
                    RESET_FONT_SCALE();
                }
                ...
                if (!hide_icon_in_layout && texture_to_draw && rect_on_screen(screen_pos, bg_max, io.DisplaySize))
                    draw_list->AddImage((void *) texture_to_draw, screen_pos, bg_max);
                ...
                if (texture_to_draw) {
                    ...
                    if (!hide_icon_in_layout && rect_on_screen(p_min, p_max, io.DisplaySize))
                ...
                if (face_uuid &&
                    ...
                    if (face_tex) {
                        ...
                        if (rect_on_screen(face_min, face_max, io.DisplaySize))
                ...
                if (has_progress_text) {
                    ...
                    ImVec2 prog_text_pos = ImVec2(prog_x_center - (progress_text_size.x * t->zoom_level) * 0.5f,
                                                  prog_y);
                    ImVec2 prog_screen_size = ImVec2(progress_text_size.x * t->zoom_level,
                                                     progress_text_size.y * t->zoom_level);
                    bool draw_progress = !hide_progress_in_layout && t->zoom_level > LOD_TEXT_SUB_THRESHOLD;
                    if (!draw_progress && tracker_layout_capture_active(t))
                        record_auto_layout_rect(t, cat->progress_pos, prog_text_pos, prog_screen_size);

                    if (!hide_progress_in_layout) {
                ...
                if (is_complex && !children_to_render.empty()) {
                    ...
                    for (TrackableItem *crit: children_to_render) {
                        ...
                        // An element with its own position is independent: only its own "Hide" toggle
                        // applies. Without one it rides along with the parent and inherits its state.
                        bool hide_crit_icon_in_layout =
                                settings->use_manual_layout &&
                                ((crit->icon_pos.is_hidden_in_layout && crit_layout_hiding_active) ||
                                 (!crit->icon_pos.is_set && parent_fully_hidden_in_layout));
                        ...
                        if (t->zoom_level > LOD_TEXT_SUB_THRESHOLD &&
                            ...
                            if (is_stat_section && (render_crit_progress || capture_crit_layout)) {
                                ...
                                if (crit_progress_text[0] != '\0') {
                                    ...
                                    ImVec2 crit_progress_screen_size = ImVec2(crit_progress_size.x * t->zoom_level,
                                                                              crit_progress_size.y * t->zoom_level);
                                    if (!render_crit_progress) {
                                        record_auto_layout_rect(t, crit->progress_pos, crit_progress_pos,
                                                                crit_progress_screen_size);
                                    } else {
    ...
    if (settings->tracker_section_custom_width_enabled[section_id]) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!should_hide_width && matches_search_width) {
                ...
                float item_name_w = tracker_cached_name_width(item->display_name, item->cached_name_w,
                                                              item->cached_name_w_font,
                                                              settings->tracker_font_size, t->tracker_font->LegacySize);
                uniform_item_width = fmaxf(uniform_item_width, fmaxf(96.0f, item_name_w));
            }
        }
        // Add default spacing ONLY in dynamic mode, 8 pixels
        uniform_item_width += horizontal_spacing;
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (is_visible_on_screen) {
            ...
            if (texture_to_draw) {
                ...
                ImVec2 p_max = ImVec2(p_min.x + scaled_size.x, p_min.y + scaled_size.y);
                // Final bottom-right for drawing
                if (!hide_item_icon_in_layout && rect_on_screen(p_min, p_max, io.DisplaySize))
                    draw_list->AddImage((void *) texture_to_draw, p_min, p_max);
    ...
    if (settings->tracker_section_custom_width_enabled[section_id]) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!should_hide_width && matches_search_width) {
                ...
                float text_width = tracker_cached_name_width(item->display_name, item->cached_name_w,
                                                             item->cached_name_w_font,
                                                             settings->tracker_font_size, t->tracker_font->LegacySize);
        ...
        if (is_visible_on_screen) {
            ...
            if (draw_cg_text || capture_cg_layout) {
                ...
                if (has_progress_text) {
                    ...
                    ImVec2 cg_prog_pos = ImVec2(prog_x_center - (progress_text_size.x * t->zoom_level) * 0.5f, prog_y);
                    ...
                    if (!draw_cg_progress) {
                        if (capture_cg_layout)
                            record_auto_layout_rect(t, item->progress_pos, cg_prog_pos, cg_prog_screen_size);
                    } else {
...
static void render_counter_goals_section(Tracker *t, const AppSettings *settings, float &current_y,
    ...
    if (settings->tracker_section_custom_width_enabled[section_id]) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            float text_width = tracker_cached_name_width(goal->display_name, goal->cached_name_w,
                                                         goal->cached_name_w_font,
                                                         settings->tracker_font_size, t->tracker_font->LegacySize);
                ...
                // Draw Progress Text
                {
                    ...
                    if (!draw_counter_progress) {
                        if (capture_counter_layout)
                            record_auto_layout_rect(t, goal->progress_pos, counter_prog_pos, counter_prog_screen_size);
                    } else {
...
static void render_multistage_goals_section(Tracker *t, const AppSettings *settings, float &current_y,
    ...
    if (settings->tracker_section_custom_width_enabled[section_id]) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!should_hide_width && (name_matches_width || stage_matches_width)) {
                ...
                float name_width = tracker_cached_name_width(goal->display_name, goal->cached_name_w,
                                                             goal->cached_name_w_font,
                                                             settings->tracker_font_size, t->tracker_font->LegacySize);
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (is_visible_on_screen) {
            ...
            ImVec2 stage_text_size = ImVec2(0, 0);
            if (show_goal_stage) {
                SET_FONT_SCALE(settings->tracker_sub_font_size, t->tracker_font->LegacySize);
                stage_text_size = ImGui::CalcTextSize(stage_text);
                RESET_FONT_SCALE();
            }
            ...
            ImVec2 ms_stage_pos = ImVec2(stage_text_x_center - (stage_text_size.x * t->zoom_level) * 0.5f,
                                         stage_text_y);
            ImVec2 ms_stage_screen_size = ImVec2(stage_text_size.x * t->zoom_level,
                                                 stage_text_size.y * t->zoom_level);
            if (t->zoom_level <= LOD_TEXT_SUB_THRESHOLD && tracker_layout_capture_active(t))
                record_auto_layout_rect(t, goal->progress_pos, ms_stage_pos, ms_stage_screen_size);
            if (t->zoom_level > LOD_TEXT_SUB_THRESHOLD) {
