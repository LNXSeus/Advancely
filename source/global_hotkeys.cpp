...
static EventHotKeyRef g_gh_mac_refs[GLOBAL_HOTKEY_MAX_SLOTS];
...
static OSStatus gh_mac_hotkey_handler(EventHandlerCallRef call_ref, EventRef event, void *userdata) {
    ...
    if (GetEventParameter(event, kEventParamDirectObject, typeEventHotKeyID, nullptr,
        ...
        hk_id.id < (UInt32) GLOBAL_HOTKEY_MAX_SLOTS) {
        gh_push_slot_event((int) hk_id.id);
    }
    return noErr;
}
...
static bool gh_platform_register(int slot_id, SDL_Scancode scancode, Uint16 mods,
    ...
    UInt32 mac_mods = 0;
    if (mods & HOTKEY_MOD_CTRL) mac_mods |= controlKey;
    if (mods & HOTKEY_MOD_SHIFT) mac_mods |= shiftKey;
    if (mods & HOTKEY_MOD_ALT) mac_mods |= optionKey;
...
typedef void *(*gh_x_set_error_handler_fn)(void *);
...
static int gh_x_error_handler(void *display, void *error_event) {
    (void) display;
    (void) error_event;
    g_gh_x_last_error = 1;
...
static void gh_x_sync_grabs(GhXDisplay *display, GhXWindow root) {
    ...
    void *prev_handler = gh_XSetErrorHandler((void *) gh_x_error_handler);
