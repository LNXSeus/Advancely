...
![Settings_Window](readme_assets/v0.9.439_Settings_Window.png)
...
| **Font Settings**     | Independently set the font for the **Tracker** (split into `Main`, `Sub` & `UI`), **Overlay**, and **UI (Settings/Notes)**. Both `.ttf` and `.otf` font files are supported. You can also adjust the **Tracker** (max 32 pt) and **UI** (max 64 pt) font sizes using sliders. _Note: Changes to Tracker and UI fonts/sizes require an application restart. Modifying the `settings.json` file directly allows for higher font sizes, but may cause overlaps._                               |
| **Overlay Settings**  | Set the `Overlay FPS Limit` independently from the tracker. Configure the `Overlay Scroll Speed` (negative values reverse the direction), the horizontal `Row 1 Icon Spacing` (in pixels), the `Sub-Stat Cycle Interval` for multi-stat animations, and toggle a constant `Speed Up Animation`. You can also align the top progress text (`Left`, `Center`, or `Right`) and toggle visibility for each section of the info bar (`World`, `Run Details`, `Progress`, `IGT`, `Update Timer`). |
