...
## Advancely is fully speedrun legal based on the [Minecraft Speedrunning rules](https://www.minecraftspeedrunning.com/public-resources/rules) by only reading from within the 'advancements', 'stats' and 'unlocks' (25w14craftmine) folders of a world that update when the game saves.
...
![Settings_Window](readme_assets/v0.9.473_Settings_Window.png)
...
| **Overlay Settings**   | Set the `Overlay FPS Limit` independently from the tracker. Configure the `Overlay Scroll Speed` (negative values reverse the direction), the horizontal `Row 1 Icon Spacing` (in pixels), enable `Custom Row 2 Spacing` or `Custom Row 3 Spacing` to set a fixed item width (in pixels) for those rows, and set the `Sub-Stat Cycle Interval` for multi-stat animations. You can also align the top progress text (`Left`, `Center`, or `Right`) and toggle visibility for each section of the info bar (`World`, `Run Details`, `Progress`, `IGT`, `Update Timer`). |
...
| **Section Item Width** | Enable and adjust the horizontal width (in pixels) for *each item* within a specific section (e.g., set "Advancements" to 150px, "Stats" to 200px). This overrides the dynamic width calculation and allows for a fixed, uniform grid. _Warning: Small values may cause text to overlap._                                                                                                                                                                                                                                                                             |
...
| `all_advancements` | 1.16.1, 1.21.6, 1.21.11                     |                  | Default, ger, zh_cn |
