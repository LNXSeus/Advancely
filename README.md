...
* **Compact Mode**: The `Row 1 Icons` section at the top can show the first-row icons (advancement criteria and
  ...
  Size` (the small parent icon overlaid on a shared criterion), `Icon Cycle Interval` (how fast the strip flips), and
  `Icon Gap Below` (space between the strip and the panel). The strip fits as many icons as the panel is wide and
